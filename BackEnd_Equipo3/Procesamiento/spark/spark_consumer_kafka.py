...
# =======================================================
# ESQUEMA ÚNICO PARA TODOS LOS SENSORES
# =======================================================
schema = StructType([
...
# =======================================================
# CAMPOS CORRECTOS POR TIPO DE SENSOR
# =======================================================
CAMPOS_POR_SENSOR = {
    "aire": ["temperature", "humidity", "co2", "pressure"],
...
def foreach_batch_insert(batch_df, batch_id):
    ...
    for row in batch_df.toLocalIterator():
        try:
            ...
            # Objeto crudo
            obj_raw = {k: getattr(obj, k) for k in obj.__fields__}

            # Filtros por tipo de sensor
            campos_validos = CAMPOS_POR_SENSOR[sensor_type]
            obj_filtrado = {k: obj_raw[k] for k in campos_validos if obj_raw[k] is not None}
            ...
            # Insertar medición en Postgres
            insert_measure_pg(pg_cur, sensor_type, pg_sensor_id, fecha_hora, obj_filtrado)

            # Insertar medición en Mongo
