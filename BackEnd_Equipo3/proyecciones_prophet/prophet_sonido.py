...
def cargar_dataset_sonido():
    csv_path = DATA_DIR / "dataset_sonido.csv"
    df = pd.read_csv(csv_path)

    # ✅ Columnas reales
    df["fecha_hora"] = pd.to_datetime(df["fecha_hora"], errors="coerce")
    df = df.dropna(subset=["fecha_hora", "laeq"])

    # ✅ Formato Prophet
    df_prophet = pd.DataFrame()
    df_prophet["ds"] = df["fecha_hora"]
    df_prophet["y"] = df["laeq"]
...
def generar_json_predicciones(df_7, df_30):
    ...
    output_path = OUTPUT_DIR / "prediccion_sonido_prophet.json"
...
def main():
    print("🚀 Entrenando Prophet para SONIDO (LAeq)...")

    df = cargar_dataset_sonido()

    pred_7 = entrenar_y_predecir(df, 7)
    pred_30 = entrenar_y_predecir(df, 30)

    generar_json_predicciones(pred_7, pred_30)
    print("✅ Proceso de SONIDO terminado correctamente.")
